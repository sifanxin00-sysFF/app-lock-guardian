...
1. 读取 `文档/守门监管_DESIGN.md`。
2. 查看项目私有工作区中的目标视觉稿和当前问题截图。
3. 在项目私有临时目录中制作静态 HTML 样板。
4. 用 390x844 截图。
5. 按 DESIGN 的 5 项评分表打分。
6. 低于 8 分先改静态原型。
7. 通过后再迁移到 `源码/AppLockApprovalWeb/src/index.ts`。
...
- 静态原型截图保存在不公开的临时工作目录。
- 生产页面截图保存在 `源码/AppLockApprovalWeb/output/playwright/`，该目录不提交。
